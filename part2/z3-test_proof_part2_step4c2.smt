...
;(assert (! (intersection u1 v1 w2 x1) :named Q12))

;w3x1 is deleted by w4x4 and x4w4, where w3 is located inside x1w1w2
(assert (! (inside x1 w1 w2 w3) :named Q21))
(assert (! (deleting w3 x1 w4 x4) :named Q22))
(assert (! (deleting w3 x1 x4 w4) :named Q23))



;w4x4 and x4w4 are not deleting w1x1 or w2x1 
(assert (! (not (deleting w1 x1 w4 x4)) :named Q101))
(assert (! (not (deleting w1 x1 x4 w4)) :named Q102))

(assert (! (not (deleting w2 x1 w4 x4)) :named Q111))
(assert (! (not (deleting w2 x1 x4 w4)) :named Q112))

(assert (! (not (inside x1 w1 w2 w4)) :named Q121))
(assert (! (not (inside x1 w1 w2 x4)) :named Q122))

(assert (! (not (intersection w3 x1 w2 x2)) :named Q131))
