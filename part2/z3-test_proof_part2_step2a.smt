...
(assert (! (intersection w3 x1 w2 x2) :named Q31))


(assert (! (not (deleting w2 x1 x3 w3)) :named Q101))

(assert (! (not (deleting w3 x1 x2 w2)) :named Q111))

(assert (! (not (deleting w1 x1 w3 x2)) :named Q121))
(assert (! (not (deleting w1 x1 w3 x3)) :named Q122))
