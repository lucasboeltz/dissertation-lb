...
;no vertex is located on the edge u1v1
...
;w2x2 and x2w2 are deleting w1v1
(assert (! (deleting w1 v1 w2 x2) :named Q11))
(assert (! (deleting w1 v1 x2 w2) :named Q12))

;w2x2 is not deleting u1v1
(assert (! (not (intersection u1 v1 w2 x2)) :named Q101))


;w2 is not inside a triangle formed by u1,v1 and w1
(assert (! (not (inside u1 v1 w1 w2)) :named Q201))
(assert (! (not (inside v1 u1 w1 w2)) :named Q202))
(assert (! (not (inside u1 v1 w1 x2)) :named Q203))
(assert (! (not (inside v1 u1 w1 x2)) :named Q204))
