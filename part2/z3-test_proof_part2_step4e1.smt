...
;w3x1 is deleted by w4x4 and w3 is located inside w1x1w2
(assert (! (inside x1 w1 w2 w3) :named Q21))
(assert (! (deleting w3 x1 w4 x4) :named Q22))

;w4x1 is intersecting w2x2
(assert (! (intersection w4 x1 w2 x2) :named Q31))



;w4x2 and w4x4 are not deleting w1x1 or w1w2 
(assert (! (not (deleting w1 x1 w4 x4)) :named Q101))
(assert (! (not (deleting w1 x1 w4 x2)) :named Q102))

(assert (! (not (deleting w2 x1 w4 x4)) :named Q111))
(assert (! (not (deleting w2 x1 w4 x2)) :named Q112))

(assert (! (not (intersection w3 x1 w2 x2)) :named Q121))

(assert (! (not (deleting w3 x1 x4 w4)) :named Q131))
