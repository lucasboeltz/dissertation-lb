...
;w3x1 is deleted by w4x4 and x4w4, where w3 is located inside w1x1w2
(assert (! (inside w1 x1 w2 w3) :named Q21))
(assert (! (deleting w3 x1 w4 x4) :named Q22))
(assert (! (deleting w3 x1 x4 w4) :named Q23))



;w4x4 is not deleting w1w2 
...
(assert (! (not (inside w1 x1 w2 w4)) :named Q121))
(assert (! (not (inside w1 x1 w2 x4)) :named Q122))
