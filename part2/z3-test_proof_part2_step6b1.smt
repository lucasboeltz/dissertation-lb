...
;u1v1 is deleted by w1x1
;(assert (! (deleting u1 v1 w1 x1) :named Q1))

;w1x1 is deleted by w2x2
(assert (! (deleting w1 x1 w2 x2) :named Q11))

;w2x1 is deleted by w3x3
(assert (! (deleting w2 x1 w3 x3) :named Q21))

;w3x1 is deleted by w2x2
(assert (! (deleting w3 x1 w2 x2) :named Q31))
;(assert (! (intersection u1 v1 x2 x1) :named Q32))
;(assert (! (deleting w1 x1 x2 w2) :named Q33))

;x2x1 is deleted by w4x4 and w4 is located inside the triangle w1x1x2
(assert (! (deleting x2 x1 w4 x4) :named Q41))
(assert (! (inside w1 x1 x2 w4) :named Q42))
;(assert (! (intersection u1 v1 w4 x1) :named Q43))

;w4x1 is deleted by w5x5
(assert (! (deleting w4 x1 w5 x5) :named Q51))
;(assert (! (intersection u1 v1 w5 x1) :named Q52))

;w5x1 is deleted by w2x2
(assert (! (intersection w5 x1 w2 x2) :named Q61))




(assert (! (not (deleting w5 x1 w4 x4)) :named Q101))
