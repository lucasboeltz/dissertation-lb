...
(assert (! (inside x1 w1 w2 w3) :named Q22))

;w3x1 is deleted by w4x4
(assert (! (deleting w3 x1 w4 x4) :named Q31))

;w4x1 is deleted by x3w3
(assert (! (deleting w4 x1 x3 w3) :named Q41))


;This implies that

;w1x1 is deleted by x3x4
(assert (! (not (deleting w1 x1 x3 x4)) :named Q101))

;or 
;w2x1 is deleted by x3x4
(assert (! (not (deleting w2 x1 x3 x4)) :named Q111))
